...
# -----------------------------------------------------------------------------
def _evaluate_combo_gpu(params, model_class, x_np, y_np, aux_np_list, cv, seed, gpu_id):
    """
    Evaluate one hyperparameter combination via CV on a dedicated GPU process.

    Args:
        params (dict): one hyperparameter combination
        model_class (nn.Module): the model class to instantiate
        x_np (ndarray): training features, shape (N, D)
        y_np (ndarray): main-task labels, shape (N,)
        aux_np_list (list of ndarray): auxiliary targets [aux1, aux2, aux3], each shape (N,)
        cv (int): number of cross-validation folds
        seed (int): random seed for reproducibility
        gpu_id (int): which CUDA device index to expose to this process

    Returns:
        (params, mean_auc, elapsed_time_s)
    """
    # 1) Limit this process to a single GPU
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # print(f"[DEBUG] PID={os.getpid():5d} → CUDA_VISIBLE_DEVICES={os.environ['CUDA_VISIBLE_DEVICES']}")
    print(f"{time.strftime('%H:%M:%S')} [DEBUG] PID={os.getpid():5d} → GPU={os.environ['CUDA_VISIBLE_DEVICES']}")
    ...
    # 3) Move data onto 'device'
    X = torch.tensor(x_np, dtype=torch.float32, device=device)
    y = torch.tensor(y_np, dtype=torch.float32, device=device)
    aux_ts = [torch.tensor(a, dtype=torch.float32, device=device) for a in aux_np_list]
    ...
    for train_idx, val_idx in kf.split(x_np):
        ...
        a_tr = [a[train_idx] for a in aux_ts]
        a_va = [a[val_idx] for a in aux_ts]

        # 4a) Instantiate and move model to GPU
        init_kwargs = {}
        if "input_dim" in sig:
        ...
        # 4b) Train with auxiliary losses
        model.train_model(
            x_tr, y_tr,
            aux1_y=a_tr[0], aux2_y=a_tr[1], aux3_y=a_tr[2],
            x_val=x_va, main_y_val=y_va,
            aux1_val=a_va[0], aux2_val=a_va[1], aux3_val=a_va[2],
            early_stopping_patience=30,
            record_loss=True
        )
        # all_val_losses.append(fold_val_losses)

        # 4c) Evaluate AUC on validation fold
        model.eval()
        with torch.no_grad():
            main_out = model.forward(x_va)[0]
        auc = roc_auc_score(y_va.cpu().numpy(), main_out.cpu().numpy())
        fold_aucs.append(auc)

    mean_auc = float(np.mean(fold_aucs))

    # min_len = min(len(v) for v in all_val_losses)
    # trimmed = [v[:min_len] for v in all_val_losses]
    # mean_val_loss = np.mean(trimmed, axis=0)

    # plt.figure(figsize=(8,5))
    # plt.plot(mean_val_loss, label='CV Mean Val BCE Loss')
    # plt.xlabel('Epoch')
    # plt.ylabel('BCE Loss')
    # plt.title(f'CV Mean Val Loss — params={params}')
    # plt.legend()
    # plt.tight_layout()
    # os.makedirs('new_img', exist_ok=True)
    # combo_str = '_'.join(f"{k}{v}" for k, v in params.items())
    # plt.savefig(f"new_img/{model_class.__name__}_{combo_str}.png")
    # plt.close()

    elapsed = time.perf_counter() - start_time
    return params, mean_auc, elapsed
...
def run_experiments_multitask(
    ...
    df_pool = train_x_pool.cpu().numpy()
    labels = train_y_pool.cpu().numpy()
    pos_idx = np.where(labels == 1)[0].tolist()
    neg_idx = np.where(labels == 0)[0].tolist()
    test_size = len(test_x_pool)
    sig = inspect.signature(model_class.__init__).parameters
    ...
    for run in range(1, num_runs + 1):
        # 1) Balanced sampling
        avail_pos = [i for i in pos_idx if i not in used]
        avail_neg = [i for i in neg_idx if i not in used]
        sample_p = random.sample(avail_pos, 250)
        sample_n = random.sample(avail_neg, 250)
        train_idx = sample_p + sample_n
        used.update(train_idx)  

        # print(f"[{method_name}] Run {run} - First 50 training indices: {train_idx[:50]}")

        x_tr = train_x_pool[train_idx]
        y_tr = train_y_pool[train_idx]
        a1_tr = train_aux1_pool[train_idx]
        a2_tr = train_aux2_pool[train_idx]
        a3_tr = train_aux3_pool[train_idx]

        # 2) Sample test subset
        test_idx = random.sample(range(test_size), 125)
        x_te = test_x_pool[test_idx]
        y_te = test_y_pool[test_idx]
        ...
        # 3) Parallel grid-search across GPUs
        best_auc, best_params = -np.inf, None
        # ctx = multiprocessing.get_context("spawn")
        with ProcessPoolExecutor(max_workers=len(gpu_ids), mp_context=ctx) as exe:
            futures = []
            ...
            for fut in as_completed(futures):
                params, auc, t_s = fut.result()
                print(f"[{method_name}] Run {run} - Combo {params} → AUC={auc:.4f}, time={t_s:.1f}s")
                if auc > best_auc:
                    best_auc, best_params = auc, params

        print(f"[{method_name}] Run {run} - Best combo {best_params}, CV AUC={best_auc:.4f}")
        best_params_list.append(best_params)

        # 4) Retrain on full 500-sample set with best_params
        init_kwargs = {}
        if "input_dim" in sig:
            init_kwargs["input_dim"] = x_tr.shape[1]
        ...
        train_out = model.train_model(
            x_tr, y_tr,
            aux1_y=a1_tr, aux2_y=a2_tr, aux3_y=a3_tr,
            x_val=x_te,  main_y_val=y_te,
            early_stopping_patience=30,
            record_loss=True
        )
        if isinstance(train_out, tuple) and len(train_out) == 3:
            _, train_losses, val_losses = train_out
        else:
            train_losses, val_losses = train_out

        # save_loss_curve(train_losses, val_losses, method_name, run)

        # 5) Evaluate on test subset
        metrics = model.evaluate(x_te, y_te)
        print(f"[{method_name}] Run {run} - Test metrics: {metrics}")
