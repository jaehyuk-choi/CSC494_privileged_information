...
# SIDE_INFO_PATH = os.path.join(PROJECT_ROOT, "prompting", "augmented_data_70B.csv")
...
#     def __init__(self, dataset_id=891, side_info_path=SIDE_INFO_PATH):
#         """
#         Args:
...
def main():
    ...
    # 5. Loop through each experiment
    for cfg in experiments:
        ...
        # perform multi-run grid search + retraining + evaluation
        results, best_params_list = run_experiments_multitask(
            model_class=Model,
            method_name=name,
            num_runs=10,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run either a single hyperparameter combo via SBATCH or all combos locally"
    )
    parser.add_argument(
        "--param_json",
        type=str,
        help="JSON string containing keys: name, model, params[, seed]"
    )
    parser.add_argument(
        "--num_runs",
        type=int,
        default=1,
        help="Number of resampling runs when running a single combo"
    )
    args = parser.parse_args()
    ...
    if args.param_json:
        # --- SBATCH mode: run exactly one hyperparameter combination ---
        ...
        # If seed provided, set it for reproducibility
        seed = cfg.get("seed", 42)
        set_seed(seed)

        # Map model string to class
        model_map = {
            "MultiTaskNN": MultiTaskNN,
            "MultiTaskNN_PretrainFinetuneExtended": MultiTaskNN_PretrainFinetuneExtended,
            "MultiTaskNN_Decoupled": MultiTaskNN_Decoupled
        ...
        # Convert single param dict to param_grid format
        single_grid = {k: [v] for k, v in params.items()}

        # Run the experiment for the specified number of runs (default 1)
        results, _ = run_experiments_multitask(
            model_class=Model,
            method_name=name,
        ...
        # Write out each run's results to CSV
        for metrics in results:
            write_results_csv(
                filename="multitask_results.csv",
                method=name,
        ...
        # ---- Local mode: run all experiments defined in main() ----
        main()
